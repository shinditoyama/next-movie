...
import { getMovieWithGenre, requests } from "@/utils/request";
...
export default function Movie({ genres }: Props) {
  ...
  const { data, isLoading, isError, isSuccess } = useQuery({
    queryKey: ["movie", genreParams, pageParams],
    queryFn: async () => {
      const movies = getMovieWithGenre(Number(genreParams), Number(pageParams));
      ...
      <section>
        ...
        <div className="w-full rounded flex flex-col md:flex-row bg-white dark:bg-gray-800">
          ...
          <div className="md:w-2/3 lg:w-4/5 px-4 py-4">
            ...
            {data?.results.length === 0 && (
              <div>Busque por um Filme, ao selecionar um gênero.</div>
            )}
          </div>
        </div>
      </section>
